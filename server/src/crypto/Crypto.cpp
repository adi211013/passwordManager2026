...
namespace
{
    ...
    unsigned char g_serverMasterKey[crypto_aead_xchacha20poly1305_ietf_KEYBYTES];
}
...
namespace Crypto
{
    void init(const std::string& jwtSecret, const std::string& masterKey)
    {
        ...
        g_jwtSecret = jwtSecret;
        if (masterKey.length() != crypto_aead_xchacha20poly1305_ietf_KEYBYTES)
        {
            throw std::runtime_error("MASTER_KEY w .env musi miec dokladnie " +
                std::to_string(crypto_aead_xchacha20poly1305_ietf_KEYBYTES) + " bajtow!");
        }
        std::memcpy(g_serverMasterKey, masterKey.data(), crypto_aead_xchacha20poly1305_ietf_KEYBYTES);
    }
    ...
    std::string generateToken(const std::string& login)
    {
        auto token = jwt::create().set_issuer("passwordManager2026")
                                  ...
                                  .sign(jwt::algorithm::hs256{g_jwtSecret});
        return token;
    }
    ...
    std::string verifyToken(const std::string& token)
    {
        try
        {
            ...
            auto verifier = jwt::verify().allow_algorithm(jwt::algorithm::hs256{g_jwtSecret}).with_issuer(
                "passwordManager2026");
    ...
    std::string encrypt(const std::string& plaintext)
    {
        ...
        crypto_aead_xchacha20poly1305_ietf_encrypt(
            ...
            nullptr, 0, nullptr, nonce, g_serverMasterKey
        );
    ...
    {
        std::string hexToDecode = ciphertextHex;
        if (hexToDecode.length() >= 2 && hexToDecode[0] == '\\' && hexToDecode[1] == 'x')
        {
            hexToDecode = hexToDecode.substr(2);
        }
        std::vector<unsigned char> combined(hexToDecode.length() / 2);
        if (sodium_hex2bin(combined.data(), combined.capacity(), hexToDecode.c_str(), hexToDecode.length(), nullptr,
                           nullptr, nullptr) != 0)
        {
        ...
        if (crypto_aead_xchacha20poly1305_ietf_decrypt(
            ...
            nullptr, c, clen,
            nullptr, 0, nonce, g_serverMasterKey) != 0)
        {
