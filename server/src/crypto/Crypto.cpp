...
#include <iostream>
static std::string g_jwtSecret;
static unsigned char g_serverMasterKey[crypto_aead_xchacha20poly1305_ietf_KEYBYTES];
void Crypto::init(const std::string& jwtSecret, const std::string& masterKey)
{
    if (sodium_init() < 0)
...
std::string Crypto::verifyToken(const std::string& token)
{
    try
    {
        ...
        auto verifier = jwt::verify().allow_algorithm(jwt::algorithm::hs256{g_jwtSecret}).with_issuer("passwordManager2026");
...
std::string Crypto::decrypt(const std::string& ciphertextHex)
{
    ...
    if (sodium_hex2bin(combined.data(), combined.capacity(), ciphertextHex.c_str(), ciphertextHex.length(), nullptr,
                       nullptr, nullptr) != 0)
    {
